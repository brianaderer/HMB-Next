...
export default function CoreParagraph(props) {
    ...
    return (
        <Text ref={paragraphRef} tag={'p'} className={`${isFirst ? 'rounded-tl-3xl pt-6 lg:pt-8 border-t-4 border-t-base-100 z-10' : ''} ${isLast ? 'rounded-br-3xl pb-6 lg:pb-8 mb-4' : ''} px-6 xl:px-16 xl:indent-8 p-4 bg-neutral text-neutral-content drop-shadow-lg`}>{attributes?.content}</Text>
    );
}
