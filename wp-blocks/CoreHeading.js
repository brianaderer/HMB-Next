...
export default function CoreHeading(props) {
    ...
    return (
        <Text ref={heading} tag={`h${level}`} className={`text-xl pl-12 ${paragraphHeading ? `rounded-tl-3xl bg-neutral text-neutral-content p-4 pb-10 relative top-6` : ''} ${alignLookup[textAlign]}`}>{attributes.content}</Text>
    );
}
