...
const VTab = props => {
    ...
    return(
        <button type={type} style={style} className={`h-48 px-3 btn btn-outline btn-secondary rounded-r-none border-r-2 border-r-secondary/40 group bg-accent ${!expanded ? `` : ''} ${className}`} onClick={event => callback(event)}>
