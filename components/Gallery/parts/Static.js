    ...
    return (
        <>
            ...
            <div className={`relative w-full my-8 mx-auto ${className}`}>
                <div className={`flex flex-row justify-center gap-2 xl:gap-4 ${oversize ? `xl:w-[110%] xl:translate-x-[-50%] xl:ml-[50%] ` : ''} items-stretch mx-auto ${background ? `bg-neutral border-2 border-secondary/20` : ''} rounded-lg drop-shadow-md p-2 xl:p-8`}>

                    {
                        Object.keys(imageGallery).map( index => {
                            ...
                            return(
                                <div key={index} className={`shrink grow w-auto basis-0 drop-shadow-md flex items-center justify-center`}>
